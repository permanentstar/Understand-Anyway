...
1. 走 `local-release-verification.md §3.1 ~ §3.3` 启动 Verdaccio，用 `scripts/release.mjs` 发布所有 OSS workspace 包。
...
| 步骤 | 命令 | 预期 |
|------|------|------|
| 包发布 | `node scripts/release.mjs patch --skip-git --registry "$LOCAL_REGISTRY"` | 所有 OSS workspace 包发布成功 |
...
- Verdaccio storage 中所有 OSS workspace 包均出现本次 release script 目标版本且未触发任何 401/403。
