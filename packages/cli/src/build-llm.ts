 ...
 * factory export (PROVIDER_FACTORY_EXPORTS.llm). The package name comes only
 * from `--llm-provider` or `providers.llm.package`; the factory config comes
 * from `providers.llm.config`. When analysis is disabled, no provider is loaded.
 */

import type { LlmProvider, LlmProviderFactory, ResolvedConfig } from "@understand-anyway/plugin-api";
...
export interface BuildLlmProviderOptions {
  ...
  /** Resolved deploy config; `providers.llm.config` is passed to the factory. */
...
export async function buildLlmProvider(options: BuildLlmProviderOptions): Promise<LlmProvider | undefined> {
  ...
  if (!packageName) {
    throw new ArgsError("--llm-analysis requires an LLM provider package (--llm-provider <pkg> or providers.llm.package)");
  }
